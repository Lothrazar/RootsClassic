...
public class GrowthPowderItem extends Item {
  ...
  public static boolean applyGrowthHere(Level level, BlockPos pos) {
    ...
    if (state.is(Blocks.DIRT)) {
      ...
      return true;
        ...
        level.isEmptyBlock(pos.above())) {
          level.setBlockAndUpdate(pos.above(), Blocks.LILY_PAD.defaultBlockState());
        }
    else {
      if (state.getBlock() instanceof BonemealableBlock igrowable) {
        if (!igrowable.isValidBonemealTarget(level, pos, state)) {
          return false;
        }
  ...
  public static void spawnGrowthParticle(Level levelAccessor, Player player) {
    ...
    for (int i = 0; i < 40; i++) {
      double velX = (lookVec.x * 0.75) + 0.5 * (levelAccessor.getRandom().nextDouble() - 0.5);
      double velY = (lookVec.y * 0.75) + 0.5 * (levelAccessor.getRandom().nextDouble() - 0.5);
      double velZ = (lookVec.z * 0.75) + 0.5 * (levelAccessor.getRandom().nextDouble() - 0.5);
