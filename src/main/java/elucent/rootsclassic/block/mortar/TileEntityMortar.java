...
public class TileEntityMortar extends TEBase {
  ...
  private boolean tryActivateRecipe(EntityPlayer player, IBlockState state) {
    ...
    if (recipe == null) {
      player.sendStatusMessage(new TextComponentString(Roots.lang("roots.mortar.invalid")), true);
      return false;
    }
    else if (recipe.isDisabled()) {
      player.sendStatusMessage(new TextComponentString(Roots.lang("roots.mortar.disabled")), true);
      return false;
    }
    else if (recipe.needsMixin() && ComponentRecipe.getModifierCapacity(inventory) < 0) {
      player.sendStatusMessage(new TextComponentString(Roots.lang("roots.mortar.mixin")), true);
      return false;
    }
    if (!world.isRemote) {
