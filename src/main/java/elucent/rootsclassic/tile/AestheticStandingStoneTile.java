...
public class AestheticStandingStoneTile extends TEBase implements ITickableTileEntity {
  ...
  public ActionResultType activate(World world, BlockPos pos, BlockState state, PlayerEntity player, Hand hand, ItemStack heldItem, BlockRayTraceResult hit) {
    if (heldItem != null) {
      int amount = 5;
      if (heldItem.getItem().is(Tags.Items.DYES_RED)) {
        if (r < 255) {
          r += amount;
          return ActionResultType.SUCCESS;
        }
      }
      if (heldItem.getItem().is(Tags.Items.DYES_GREEN)) {
        if (g < 255) {
          g += amount;
          return ActionResultType.SUCCESS;
        }
      }
      if (heldItem.getItem().is(Tags.Items.DYES_BLUE)) {
        if (b < 255) {
          b += amount;
          return ActionResultType.SUCCESS;
        }
      }
      if (heldItem.getItem().is(Items.DYES_WHITE)) {
        r = 0;
        g = 0;
        b = 0;
        return ActionResultType.SUCCESS;
