...
public class GuiTablet extends GuiScreen {
  ...
  public void drawScreen(int mouseX, int mouseY, float partialTicks) {
    ...
    for (int i = 0; i < ResearchManager.globalResearches.get(currentGroup).researches.size(); i++) {
      ...
      if (mouseX >= basePosX + 32 * xShift && mouseX < basePosX + 32 * xShift + 24 && mouseY >= 32 + 40 * yShift && mouseY < 32 + 40 * yShift + 24) {
        String name = Roots.lang("roots.research." + ResearchManager.globalResearches.get(currentGroup).name + "." + ResearchManager.globalResearches.get(currentGroup).researches.get(i).name + ".name");
    ...
    this.fontRenderer.drawStringWithShadow(Roots.lang("roots.research." + ResearchManager.globalResearches.get(currentGroup).name + ".name"), width / 2.0f - (fontRenderer.getStringWidth(ResearchManager.globalResearches.get(currentGroup).properName) / 2.0f), height - 16.0f, Util.intColor(255, 255, 255));
    Minecraft.getMinecraft().getTextureManager().bindTexture(Const.tabletGui);
    if (mouseX >= 32 && mouseX < 64 && mouseY >= height - 48 && mouseY < height - 32) {
